...
public class MyPartyData_importer : AssetPostprocessor {
	private static readonly string filePath = "Assets/ExcelData/MyPartyData.xls";
	private static readonly string exportPath = "Assets/ExcelData/MyPartyData.asset";
	private static readonly string[] sheetNames = { "MyPartyData", };
	...
	static void OnPostprocessAllAssets (string[] importedAssets, string[] deletedAssets, string[] movedAssets, string[] movedFromAssetPaths)
	{
		foreach (string asset in importedAssets) {
			...
			MyPartyData data = (MyPartyData)AssetDatabase.LoadAssetAtPath (exportPath, typeof(MyPartyData));
			if (data == null) {
				data = ScriptableObject.CreateInstance<MyPartyData> ();
			...
			using (FileStream stream = File.Open (filePath, FileMode.Open, FileAccess.Read, FileShare.ReadWrite)) {
				...
				foreach(string sheetName in sheetNames) {
					...
					MyPartyData.Sheet s = new MyPartyData.Sheet ();
					s.name = sheetName;
					...
					for (int i=1; i<= sheet.LastRowNum; i++) {
						...
						MyPartyData.Param p = new MyPartyData.Param ();
